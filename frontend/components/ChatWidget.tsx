...
export function ChatWidget() {
  ...
  return <Box sx={{ position: "fixed", right: { xs: 12, md: 28 }, bottom: { xs: "max(12px, env(safe-area-inset-bottom))", md: 28 }, zIndex: 1400 }}>
    {open && <Paper elevation={24} sx={{ position: { xs: "fixed", sm: "absolute" }, inset: { xs: 10, sm: "auto" }, right: { sm: 0 }, bottom: { sm: 76 }, width: { xs: "auto", sm: 430 }, height: { xs: "calc(100dvh - 20px)", sm: 640 }, maxHeight: { sm: "calc(100dvh - 108px)" }, borderRadius: { xs: 4, sm: 5 }, overflow: "hidden", display: "flex", flexDirection: "column", bgcolor: "#111521", border: "1px solid rgba(124,108,255,.32)", boxShadow: "0 28px 90px rgba(0,0,0,.55)" }}>
      ...
      <Paper component="form" onSubmit={submit} square elevation={0} sx={{ p: 1.25, display: "flex", gap: 0.65, alignItems: "center", bgcolor: "rgba(12,15,24,.98)", borderTop: "1px solid rgba(255,255,255,.09)" }}><input ref={uploadRef} hidden type="file" accept="image/png,image/jpeg,image/webp" onChange={upload} /><Tooltip title="Search with a photo"><IconButton aria-label="Upload product photo" onClick={() => uploadRef.current?.click()} disabled={loading} sx={{ bgcolor: "rgba(255,255,255,.06)" }}><AddPhotoAlternateRoundedIcon /></IconButton></Tooltip><Tooltip title={listening ? "Stop listening" : "Shop with your voice"}><IconButton color={listening ? "error" : "default"} aria-label={listening ? "Stop voice input" : "Start voice input"} onClick={toggleListening} disabled={loading} sx={{ bgcolor: "rgba(255,255,255,.06)" }}>{listening ? <StopCircleRoundedIcon /> : <MicRoundedIcon />}</IconButton></Tooltip><TextField fullWidth size="small" placeholder={listening ? "Listening…" : "Ask Emma anything…"} value={input} onChange={(event) => setInput(event.target.value)} sx={{ "& .MuiOutlinedInput-root": { borderRadius: 3, bgcolor: "rgba(255,255,255,.045)" } }} /><IconButton type="submit" color="primary" aria-label="Send message" disabled={!input.trim() || loading} sx={{ width: 42, height: 42, bgcolor: "primary.main", color: "primary.contrastText", "&:hover": { bgcolor: "primary.dark" }, "&.Mui-disabled": { bgcolor: "rgba(255,255,255,.07)" } }}><SendRoundedIcon /></IconButton></Paper>
    </Paper>}
